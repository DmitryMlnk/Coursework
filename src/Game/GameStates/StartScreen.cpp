...
StartScreen::StartScreen(const std::vector<std::string> &startScreenDescription)
        ...
        m_tankSprite(std::make_pair(
                ResourceManager::getSprite("player1_yellow_tank_type1_sprite_right"),
                glm::vec2(8 * BLOCK_SIZE,m_menuSprite.second.y + 6 * BLOCK_SIZE - m_currentMenuSelection * 2 * BLOCK_SIZE))),m_tankSpriteAnimator(m_tankSprite.first) {
...
void StartScreen::render() const {
    ...
    m_backgroundSprite.first->render(m_backgroundSprite.second, glm::vec2 (STARTSCREEN_WIDTH + BLOCK_SIZE * 4, STARTSCREEN_HEIGHT), 0.f, -0.1f);
